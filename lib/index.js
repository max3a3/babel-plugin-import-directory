...
var getFiles = function getFiles(parent) {
  ...
  for (var i = 0, l = r.length; i < l; i++) {
    var child = r[i];
    var _path$parse = _path2["default"].parse(child), name = _path$parse.name, ext = _path$parse.ext;
    ...
    visitor: {
      ImportDeclaration: function ImportDeclaration(path, state) {
        ...
        var files = _files.map(function (file) {
          return [file, nameTransform(file[file.length - 1]), path.scope.generateUidIdentifier(file[file.length - 1])]
        });
        if (!files.length) {
          return
        }
        var imports = files.map(function (_ref) {
          var _ref2 = _slicedToArray(_ref, 3), file = _ref2[0], fileName = _ref2[1], fileUid = _ref2[2];
          var requirePath = _path2["default"].join.apply(_path2["default"], [cleanedPath].concat(_toConsumableArray(file)));
          // console.log("** requirePath", requirePath);
          // requirePath = cleanedPath + "_X_" + file;// .replace('\\','/') // windows fix
          requirePath =  requirePath.replace('\\','/') // windows fix
          return t.importDeclaration([t.importNamespaceSpecifier(fileUid)], t.stringLiteral(pathPrefix + requirePath))
        });
        ...
        if (isExplicitWildcard) {
          files.forEach(function (_ref3) {
            var _ref4 = _slicedToArray(_ref3, 3), file = _ref4[0], fileName = _ref4[1], fileUid = _ref4[2];
          ...
          files.forEach(function (_ref5) {
            var _ref6 = _slicedToArray(_ref5, 3), file = _ref6[0], fileName = _ref6[1], fileUid = _ref6[2];
